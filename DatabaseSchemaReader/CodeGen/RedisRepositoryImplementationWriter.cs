...
{
    public class RedisRepositoryImplementationWriter
    {
        ...
        public RedisRepositoryImplementationWriter(DatabaseSchema schema, CodeWriterSettings codeWriterSettings, IEnumerable<string> logicalDeleteColumns)
        {
        ...
        private void WriteConstructorsAndFields()
        {
            ...
            {
                DataType = "ConnectionMultiplexer",
                Summary = "Represents an inter-related group of connections to Redis servers.",
                Name = "connection"
            });
            fields.Add(new Parameter
            {
                DataType = $"ILogger<{CodeWriterUtils.GetRepositoryImplementationName(table)}>",
                Summary = $"A logging mechanism for objects of type <see cref=\"{CodeWriterUtils.GetRepositoryImplementationName(table)}\"/>.",
                Name = _loggerFieldName
            });
            fields.Add(new Parameter
            {
                DataType = "IServiceProvider",
                Summary = $"Defines a mechanism for retrieving instances of <see cref=\"{table.NetName}\"/>.",
        ...
        private void WriteConstructor(IEnumerable<Parameter> fields)
        {
            ...
            var className = CodeWriterUtils.GetRepositoryImplementationName(table);
            classBuilder.AppendXmlSummary($"Initializes a new instance of the <see cref=\"{className}\"/> class.", parameters: fields);
            using (classBuilder.BeginNest($"public {className}({constructorSignature})"))
            {
                foreach (var f in fields)
                {
        ...
        private void WriteUsings()
        {
            ...
            classBuilder.AppendLine("using StackExchange.Redis;");
            classBuilder.AppendLine("");
        }
        ...
        private void WriteGetListMethodSummary(IEnumerable<Parameter> methodParameters)
        {
            classBuilder.AppendXmlSummary(
                $"Queries the data store for each instance.",
        ...
        {
            classBuilder.AppendXmlSummary(
                $"Queries the data store for each instance whose properties match the specified values.",
        ...
        {
            classBuilder.AppendXmlSummary(
                $"Deletes the specified <see cref=\"{table.NetName}\"/> from the data store.",
                ...
                {
                    new Tuple<string, string>("EntityNotFoundException", "<paramref name=\"entity\"/> is not found in the data store."),
                    new Tuple<string, string>("EntityHasDependenciesException", "<paramref name=\"entity\"/> cannot be deleted because it is still referenced in the data store.")
                },
                methodParameters);
        }
        ...
        {
            classBuilder.AppendXmlSummary(
                $"Updates the specified <see cref=\"{table.NetName}\"/> in the data store.",
        ...
        {
            classBuilder.AppendXmlSummary(
                $"Inserts the specified <see cref=\"{table.NetName}\"/> to the data store.",
        ...
        {
            classBuilder.AppendXmlSummary(
                $"Queries the data store for a single instance whose properties match the specified values.",
                $"An instance of <see cref=\"{table.NetName}\"/>, or <c>null</c> if there is no match.",
                $"This method gets only primitive properties, i.e., only properties that correspond to columns on the data store table. No recursion is performed.",
                new List<Tuple<string, string>>()
                {
                    new Tuple<string, string>("ArgumentNullException", "<paramref name=\"filter\"/> is <c>null</c> or empty."),
                    new Tuple<string, string>("InvalidOperationException", "There are multiple matches in the data store.")
                },
                methodParameters);
        }
